...
function ImageCaraosal(props) {
  ...
  const next = (e) => {
    ...
    var height =
      caroSliderRef.current != null
        ? caroSliderRef.current.children[0].clientHeight
        : 0;
    var width =
      caroSliderRef.current != null
        ? caroSliderRef.current.children[0].clientWidth
        : 0;

    if (curcarInd < imgList.length) {
      curcarInd = curcarInd + 1;
      if (caroSliderRef.current != null)
  ...
  return (
    <div className="img-caraousal-container">
      <div className="img-caraousal">
        ...
        <button onClick={next} ref={nextRef} className="next">
          <span>{">"}</span>
        </button>
