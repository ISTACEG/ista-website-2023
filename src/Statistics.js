...
function Statistics() {
  ...
  const filteredPlacementData = placementsData.filter(
    (item) => item.batch === pselectedYear
  );
  const filteredInternshipData = internData.filter(
    (item) => item.batch === iselectedYear
  );
  ...
  const prepareChartData = (dataset) => {
    ...
    const pieChartPlacementData = useMemo(() => prepareChartData(filteredPlacementData), [filteredPlacementData]);
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/${range1}?key=${API_KEY}`
      )
      .then((response) => {
    ...
    axios
      .get(
        `https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/${rangeSingleCell}?key=${API_KEY}`
      )
      .then((response) => {
    ...
    axios
      .get(
        `https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/${range2}?key=${API_KEY}`
      )
      .then((response) => {
        const formatted = response.data.values.map((row) => ({
          companyName: row[0],
  ...
  return (
    <section
      id="events"
      className="events-box special-1"
      ...
      {loading ? (
        ...
        <div className="event-item-1">
          <div
            ...
            style={{ textAlign: "left", alignItems: "self-start" }}
          >
            <div className="chart-container-1">
              <div className="dropdown-container-1">
                <Select
                  options={pyearOptions}
                  onChange={handlePYearChange}
                  defaultValue={pyearOptions.find(
                    (option) => option.value === pselectedYear
                  )}
            ...
            <div className="chart-container-1">
              <div className="dropdown-container-1">
                <Select
                  ...
                  defaultValue={iyearOptions.find(
                    (option) => option.value === iselectedYear
                  )}
                  className="year-dropdown-1"
                />
              </div>
          ...
          <p>
            Note: This data only includes students from the IT Department from
